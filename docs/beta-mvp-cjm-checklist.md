...
> **АРХИВ.** Этот чек-лист писался под приёмку беты и заменён реестром путей:
> [`docs/cjm/reestr.html`](cjm/reestr.html). Он оставлен как история — по нему видно,
> что считалось готовым к бете и в каком порядке закрывалось. Актуальное состояние
> путей и открытые вопросы смотреть в реестре, не здесь: часть пунктов ниже устарела,
> и продукт с тех пор переименован из КОМПАСа в ПРАКТИКУ.
