        ...
        <main className="min-h-screen bg-[#faf8f5] px-4 py-8 text-[#1f2a24]">
            <div className="mx-auto max-w-2xl rounded-3xl border border-[#e6dfd1] bg-white p-6 shadow-sm sm:p-8">
                <div className="mb-6 border-b border-[#e6dfd1] pb-5">
                    ...
                    {/* break-words обязателен и не заменяется чисткой названия:
                        имя без подчёркиваний, но длинное, сломало бы страницу
                        так же. Правило про перенос — на разметке, чистка — на
                        читаемости. */}
                    <h1 className="text-[22px] font-bold leading-snug tracking-tight break-words sm:text-2xl">
                        {documentDisplayTitle(delivery.documentTitle, delivery.fileName)}
                    </h1>
                    <dl className="mt-3 grid grid-cols-[auto_minmax(0,1fr)] gap-x-3 gap-y-1 text-sm text-[#5d665f]">
                        <dt className="text-[#8a8f88]">Специалист</dt>
                        <dd className="min-w-0 break-words font-semibold text-[#1f2a24]">{delivery.psychologistName || 'специалист'}</dd>
                        <dt className="text-[#8a8f88]">Клиент</dt>
                        <dd className="min-w-0 break-words font-semibold text-[#1f2a24]">{delivery.clientName}</dd>
                        <dt className="text-[#8a8f88]">Версия</dt>
                        <dd className="min-w-0 break-words font-semibold text-[#1f2a24]">{delivery.documentVersion}</dd>
                    </dl>
                </div>

                {delivery.documentContent ? (
                    <article className="mb-6 max-h-[60vh] overflow-auto whitespace-pre-wrap break-words rounded-2xl border border-[#e6dfd1] bg-[#faf8f5] p-5 text-[16px] leading-7 text-[#2b332d]">{delivery.documentContent}</article>
                ) : delivery.fileUrl ? (
                    /* Имя файла ушло со строки действия на отдельную: раньше
                       «Открыть файл документа: <длинное_имя.pdf>» уезжало за
                       край, и вместе с ним пропадало само действие. */
                    <a href={openFileUrl(delivery.fileUrl)} target="_blank" rel="noreferrer" className="mb-6 flex items-start gap-3 rounded-2xl border border-[#e6dfd1] bg-[#faf8f5] p-5 text-base font-semibold text-[#1a4d3a] transition-colors hover:bg-[#f1ede4]">
                        <span aria-hidden className="leading-6">📄</span>
                        <span className="min-w-0">
                            Открыть файл документа
                            {delivery.fileName ? (
                                <span className="mt-1 block break-all text-[13px] font-normal text-[#5d665f]">{delivery.fileName}</span>
                            ) : null}
                        </span>
                    </a>
                ) : (
