    ...
    try {
        ...
        const [settings, previousSession, paymentStatus] = await Promise.all([
            ...
            db.diarySession.findFirst({
                where: { psychologistId: auth.userId, clientId: session.clientId, id: { not: session.id }, date: { lt: session.date }, status: { in: ['completed', 'confirmed'] } },
...
export async function PATCH(
    ...
    try {
        ...
        if (isReschedule) {
            ...
            const conflicts = await db.diarySession.findMany({ where: { psychologistId: auth.userId, id: { not: id }, date: { gte: dayStart, lte: dayEnd }, status: { not: 'cancelled' } } });
            ...
            for (const conflict of conflicts) {
                ...
                if (newStart < sEnd && newEnd > sStart) return NextResponse.json({ error: 'Это время уже занято другой сессией' }, { status: 409 });
            }
            autoDeleteSessionFromCalendars(auth.userId, id).catch(console.error);
            const updated = await db.diarySession.update({
                where: { id },
                data: { date: dateObj, time: newTime, endTime: newEndTime, status: body.status ? body.status.toLowerCase() : 'pending', notified24h: false, notified1h: false, ...notePatch(body) },
                include: { client: { select: { id: true, name: true } } },
            });
