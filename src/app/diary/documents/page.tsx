...
function formatDate(value: string | null) {
    if (!value) return '—';
    try { return new Date(value).toLocaleString('ru-RU', { day: '2-digit', month: '2-digit', year: '2-digit', hour: '2-digit', minute: '2-digit' }); }
    catch { return value; }
}
            ...
            {tab === 'journal' ? (
                <section className="bg-card border border-border rounded-2xl p-6 shadow-card space-y-5">
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                        <div>
                            <h2 className="text-lg font-bold text-foreground">Журнал документов и согласий</h2>
                            <p className="text-sm text-muted-foreground">Статусы sent → opened → acknowledged, версия и contentHash сохраняются для проверки 152-ФЗ.</p>
                        </div>
                    ...
                    <div className="flex flex-wrap gap-3">
                        <select value={journalConsentFilter} onChange={e => setJournalConsentFilter(e.target.value as any)} className="px-4 py-2.5 border border-border rounded-xl bg-background text-sm font-semibold">
                            <option value="all">Все согласия</option>
                            <option value="missing">Без согласия</option>
                    ...
                    {journalLoading ? (
                        <div className="py-12 text-center text-sm text-muted-foreground">Загрузка журнала...</div>
                    ) : deliveries.length === 0 ? (
                        <div className="py-12 text-center border border-dashed border-border rounded-2xl bg-muted/30 text-sm text-muted-foreground">Записей по выбранным фильтрам нет.</div>
                    ) : (
                        <div className="overflow-x-auto rounded-2xl border border-border">
                            <table className="min-w-[980px] w-full text-sm">
                                <thead className="bg-muted/50 text-left text-xs uppercase tracking-wide text-muted-foreground">
                                    <tr><th className="p-3">Клиент</th><th className="p-3">Документ</th><th className="p-3">Версия</th><th className="p-3">Статус</th><th className="p-3">Канал</th><th className="p-3">Отправлен</th><th className="p-3">Открыт</th><th className="p-3">Принят</th><th className="p-3">Hash</th></tr>
                                </thead>
                                <tbody>
                                    {deliveries.map(item => (
                                        <tr key={item.id} className="border-t border-border align-top">
                                            <td className="p-3 font-semibold text-foreground">{item.clientName}</td>
                                            <td className="p-3 text-foreground">{item.documentTitle}</td>
                                            <td className="p-3"><span className="rounded-full bg-primary/10 px-2 py-1 text-xs font-bold text-primary">v {item.documentVersion}</span></td>
                                            <td className="p-3"><span className={`rounded-full px-2 py-1 text-xs font-bold ${item.acknowledgedAt ? 'bg-green-50 text-green-700' : item.openedAt ? 'bg-amber-50 text-amber-700' : 'bg-muted text-muted-foreground'}`}>{statusLabels[item.status] || item.status}</span></td>
                                            <td className="p-3">{item.deliveryChannel}</td>
                                            <td className="p-3">{formatDate(item.sentAt)}</td>
                                            <td className="p-3">{formatDate(item.openedAt)}</td>
                                            <td className="p-3">{formatDate(item.acknowledgedAt)}</td>
                                            <td className="p-3 font-mono text-xs text-muted-foreground">{item.documentContentHash.slice(0, 12)}…</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
