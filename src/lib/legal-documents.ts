...
// АКЦЕПТ ЦЕНТРАЛЬНЫХ ДОКУМЕНТОВ ПРОДУКТ НЕ СОБИРАЕТ. Пусто — намеренно.
//
// До 11.09.2026 здесь стоял TERMS, и барьер /legal-acceptance собирал
// согласие с нашей собственной страницей cmpas.ru/legal/terms. Сторона
// СИМПАС (issue #172, 11.09.2026) показала, чем это плохо, и довод
// сильнее, чем «так написано в ТЗ»:
//
//   * текста этой страницы нет в реестре документов Экосистемы, и
//     отпечатка у неё не существует. Доказательство согласия по правилу
//     05_CONSENT_PDN.md §2.4 состоит из трёх частей — записи о событии,
//     текста редакции по неизменяемому адресу и совпадения отпечатков. У
//     наших записей есть только первая;
//   * Пользовательское соглашение человек принимает в СИМПАС, кнопкой
//     входа. Вторая запись о том же факте, с другим временем и другим
//     источником, в споре заставит объяснять, какая из двух настоящая
//     (14_LEGAL_PRODUCTS_UNIFIED.md §1).
//
// Поэтому барьер выключается СЕЙЧАС, не дожидаясь переезда страниц:
// страница как справочное чтение терпима, страница как место акцепта —
// нет. Сами страницы остаются открытыми до публикации центральных текстов
// в консент-центре: перенаправить их сегодня значило бы увести человека с
// работающего текста на заглушку.
//
// СТАРЫЕ ЗАПИСИ ОБ АКЦЕПТАХ НЕ ТРОГАЮТСЯ. Ни переписать, ни удалить, ни
// «исправить» задним числом — прямой запрет ТЗ §7 и здравого смысла:
// запись о согласии это доказательство, а не состояние.
export const ACCOUNT_REQUIRED_TYPES: LegalDocType[] = [];
