...
// тестом без Next.
//
// Принцип деградации: подсказки — удобство, а не обязательное условие. Любая
// неудача (нет токена, DaData молчит, лимит исчерпан) отдаёт пустой список,
// поле остаётся обычным текстовым, и специалист вводит адрес руками.

const DADATA_URL = 'https://suggestions.dadata.ru/suggestions/api/4_1/rs/suggest/address';

/** Короче трёх символов подсказка бессмысленна — и это лишний платный запрос. */
export const MIN_QUERY_LENGTH = 3;
/** Длиннее любого реального адреса: защита от мусора в теле запроса. */
...
export type SuggestOutcome = {
    ...
    /** Почему список пуст. Для логов и тестов; наружу не отдаётся. */
