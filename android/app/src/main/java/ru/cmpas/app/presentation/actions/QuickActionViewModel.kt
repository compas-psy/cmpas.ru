...
class QuickActionViewModel @Inject constructor(
    ...
    private suspend fun saveClient(name: String, contact: String, notes: String): String {
        ...
        return try {
            ...
            if (client != null) {
                localStore.upsertClient(client.copy(notes = notes.ifBlank { client.notes }))
                "Клиент создан и сохранён"
            } else {
                localStore.createClient(name, phone, email, notes)
                "Клиент сохранён локально"
            }
        } catch (_: Exception) {
            localStore.createClient(name, phone, email, notes)
            "Клиент сохранён локально"
    ...
    private suspend fun saveSession(client: Client?, date: String?, time: String?, formatRaw: String, comment: String): String {
        ...
        return try {
            val response = if (!client.id.startsWith("local-")) {
                api.createSession(
