...
enum class ScreenFocus(val key: String) {
    ...
    PAYMENT("payment"),
    /**
     * Карточка клиента: сразу шторка отправки документа.
     *
     * Задача 27: быстрое действие «Отправить документ» собирало сообщение
     * само и вставляло в него адрес вида /d/<id> — маршрута с
     * таким адресом в вебе нет, клиент получал битую ссылку. Настоящая
     * отправка живёт в карточке клиента: она просит сервер создать доставку
     * и подписать адрес. Быстрое действие теперь ведёт туда, а не повторяет
     * его наполовину.
     */
    DOCUMENT("document");
