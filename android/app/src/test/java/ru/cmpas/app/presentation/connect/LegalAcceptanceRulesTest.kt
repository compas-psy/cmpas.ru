...
class LegalAcceptanceRulesTest {
    ...
    @Test
    fun `на экране входа документы ПОКАЗЫВАЮТСЯ, но не принимаются`() {
        // ЧТО ИМЕННО ЗАПРЕЩЕНО. Пользовательское соглашение принимается в
        // СИМПАС, при создании учётной записи; повторный акцепт в продукте
        // создаёт вторую запись о том же факте, с другим временем и другим
        // источником. Политику не принимают вовсе — это информационный
        // документ оператора, и глагол принятия рядом с ним сам по себе
        // дефект правовой конструкции.
        //
        // ЧТО ПРИ ЭТОМ ОБЯЗАНО БЫТЬ. Ссылки. Раньше этот тест запрещал на
        // экране входа сами слова «Пользовательское соглашение» и
        // «Политика» — и запрещал заодно единственный способ их прочитать:
        // в приложении других экранов с этими документами до входа нет.
        // Веб прошёл ровно эту правку 11.09.2026 (src/app/auth/AuthForm.tsx):
        // акцепт убран, ссылки оставлены. Здесь то же самое.
        val visible = withoutComments(loginScreen)
        for (forbidden in listOf("принимаю", "Принимая", "соглашаетесь", "Продолжая, вы")) {
            assertFalse(
                "на экране входа не должно быть «$forbidden» — акцепт даётся в СИМПАС",
                visible.contains(forbidden),
            )
        }
        assertTrue("ссылка на Соглашение обязана быть", visible.contains("Пользовательское соглашение"))
        assertTrue("ссылка на Политику обязана быть", visible.contains("Политика конфиденциальности"))
    }

    @Test
    fun `адреса документов берутся у реестра, а не вписаны редакцией в сборку`() {
        // Вписанный номер редакции означает, что после выхода новой человек
        // читает старый текст, неотличимый на вид от действующего.
        val model = File("src/main/java/ru/cmpas/app/presentation/auth/LoginViewModel.kt").readText()
        assertTrue(model.contains("legalLinksFrom(documents)"))
        assertTrue(model.contains("cmpas_privacy"))
        assertFalse(
            "адрес конкретной редакции в сборку не вписывается",
            Regex("""legal/(terms|privacy)/\d""").containsMatchIn(withoutComments(model)),
        )
    }

    @Test
    fun `клавиатура не закрывает поле ввода на экране входа`() {
        // Экран был одним неподвижным столбцом по центру: клавиатура
        // выезжала поверх поля почты, и человек печатал вслепую.
        val visible = withoutComments(loginScreen)
        assertTrue("экран обязан отдавать клавиатуре высоту", visible.contains("imePadding()"))
        assertTrue("до поля надо иметь возможность доехать", visible.contains("verticalScroll("))
    }
