...
test.describe('§10 адреса', () => {
    ...
    test('/u/<slug> — постоянная ссылка ведёт к тому самому специалисту', async ({ page }) => {
        ...
        // Формат назван у каждого предложенного часа, а не угадывается: по
        // умолчанию предпочтение «после 18:00», и там кабинет. Что оба
        // формата одного дня предлагаются вместе — проверяет сценарий 3.
        await expect(body).toContainText(/Очно|Онлайн/);
