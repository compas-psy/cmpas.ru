...
tap_text_below() {
    ...
    done < <(dump | tr '>' '\n' | grep -F "$needle" | grep -o 'bounds="\[[0-9]*,[0-9]*\]\[[0-9]*,[0-9]*\]"')
    return 1
}
...
# «Аккаунт» — надпись над заголовком экрана профиля, её нет ни на одном
# другом экране.
if goto_tab "Профиль" 5 "Аккаунт" "перешёл в профиль"; then
