...
# Продукт сам поднимает часть подписей в верхний регистр (Eyebrow делает
# .uppercase()), поэтому сверка идёт без учёта регистра. Кириллица так
# сравнивается только в UTF-8-локали, и молча ошибиться тут нельзя: промах
# выглядел бы как «нет на экране».
export LC_ALL=C.UTF-8
if ! printf '%s' 'БАЗА' | grep -qiF 'База'; then
    echo "ОСТАНОВ: в этой среде grep не складывает регистр кириллицы (LC_ALL=$LC_ALL)." >&2
    echo "Без этого «нет на экране» означало бы регистр, а не продукт." >&2
    exit 2
fi
...
# Есть ли на экране текст. Сравнение по подстроке и без учёта регистра:
# подписи в Compose приходят в content-desc или в text, точное совпадение
# ловит не всё, а Eyebrow ещё и поднимает подпись в верхний регистр.
has_text() {
    dump | grep -qiF "$1"
}
...
# Доказательство маркером могло увести список вниз. Кадр, подписанный «верх»,
# обязан быть верхом, иначе подпись врёт.
scroll_to_top() {
    for _ in 1 2 3 4 5 6; do
        adb shell input swipe $((SCR_W / 2)) $((SCR_H * 30 / 100)) $((SCR_W / 2)) $((SCR_H * 75 / 100)) 300
    done
    sleep 1
}
...
# Нажать по тексту, но ТОЛЬКО в нижней части экрана. Нужно потому, что
# подписи вкладок дока повторяются в содержимом: на главном экране в
# чек-листе есть строка «Клиенты», и она идёт в выгрузке РАНЬШЕ вкладки —
# tap_text попадал в неё и уходил на полноэкранную форму «Добавить
# клиента», у которой дока нет вовсе. Дальше «Профиль» на экране,
# разумеется, не находился.
tap_text_below() {
    local needle="$1" min_y="$2"
    local nums x1 y1 x2 y2
    while read -r line; do
        nums=$(printf '%s' "$line" | grep -o '[0-9]\+')
    ...
    done < <(dump | tr '>' '\n' | grep -iF "$needle" | grep -o 'bounds="\[[0-9]*,[0-9]*\]\[[0-9]*,[0-9]*\]"')
    return 1
}

# Пять мест дока: Сегодня · Календарь · [+] · Клиенты · Профиль. Доли
# сняты с настоящего кадра 1080×2340; раскладка задана в dp, поэтому на
# узком экране те же доли.
tap_dock() {
    local frac
    case "$1" in
        1) frac=15 ;; 2) frac=33 ;; 3) frac=50 ;; 4) frac=67 ;; 5) frac=85 ;;
        *) return 1 ;;
    esac
    adb shell input tap $((SCR_W * frac / 100)) $((SCR_H * 92 / 100))
    sleep 2
}

# Переход по доку с доказательством ПО СОДЕРЖИМОМУ.
#
# Сам тап ничего не доказывает: попасть по координате можно и мимо. Поэтому
# успех засчитывается только тогда, когда после перехода на экране нашёлся
# маркер целевого экрана — при необходимости после прокрутки.
goto_tab() {
    local label="$1" pos="$2" marker="$3" what="$4"
    tap_text_below "$label" $((SCR_H * 88 / 100)) || tap_dock "$pos"
    sleep 1
    if has_text "$marker" || scroll_to_text "$marker" 4; then
        ok "$what (на экране найдено «$marker»)"
        return 0
    fi
    fail "$what — после перехода «$marker» на экране нет"
    return 1
}
...
# Маркер — «Неделя»: один из режимов календаря. Слово «Календарь» маркером
# быть не может, оно же и подпись вкладки — проверка доказывала бы сама себя.
goto_tab "Календарь" 2 "Неделя" "перешёл в календарь"
shot "calendar-A05" 3
...
# Маркер «База · N человек» — надпись именно списка клиентов, а не строки
# чек-листа с тем же словом и не формы добавления.
goto_tab "Клиенты" 4 "База" "перешёл в список клиентов"
shot "clients-list" 3

if tap_text "Анастасия" || tap_text "Пётр"; then
...
# Маркер профиля — заголовок группы «Аналитика»: он есть на этом экране и
# больше нигде во всём приложении, и рисуется безусловно. Соседняя группа
# «Автонапоминания клиенту» маркером быть не может: она появляется только
# после ответа сервера, и её отсутствие говорило бы о запросе, а не о
# переходе. Слово «Профиль» — тем более: это подпись самой вкладки, и
# проверка доказывала бы сама себя.
if goto_tab "Профиль" 5 "Аналитика" "перешёл в профиль"; then
    scroll_to_top
    ...
    for group in "Мессенджеры и данные"; do
        if has_text "$group"; then ok "A04: группа «$group» подписана"; fi
    done
    # Прокрутка до «Практики» останавливается, едва заголовок показался у
    # нижнего края, — сами строки группы лежат ещё ниже (видно на кадре
    # profile-A04-группы). До строки надо долистать отдельно, иначе «не
    # нашёл» скажет о высоте экрана, а не о продукте.
    if ! scroll_to_text "Кабинеты" 4; then
        fail "A08: строки «Кабинеты» нет в группе «Практика» даже после прокрутки"
    elif step "Кабинеты" "нажал «Кабинеты» в группе «Практика»"; then
        sleep 1
        shot "cabinets-A08" 3
        # Подзаголовок самого экрана кабинетов. Слово «Кабинет» маркером быть
        # не может: строка «Кабинеты» есть и в профиле, откуда мы нажимали, —
        # проверка проходила бы и не открыв экран. Берём начало подзаголовка,
        # а не весь: он в одну строку с многоточием, и на узком экране хвост
        # обрезается — искать целиком значило бы проверять ширину экрана.
        if has_text "Места, куда"; then
            ok "A08: экран кабинетов открылся"
        else
            fail "A08: экран кабинетов не открылся — его подзаголовка на экране нет"
        fi
        adb shell input keyevent KEYCODE_BACK; sleep 2
    fi
fi
