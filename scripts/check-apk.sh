...
if [ -z "$TREE" ]; then
    ...
    echo "  Яндекс ID, идентификатор приложения: ${YANDEX_ID:-нет в манифесте}"
    echo "  ВК, идентификатор приложения:        ${VK_ID:-нет в манифесте}"
    # Схема возврата ВК складывается из идентификатора — если он не тот,
    # ответ придёт в схему, которую никто не слушает.
    [ -n "$VK_ID" ] && echo "  ВК, схема возврата:                  vk${VK_ID}://vk.ru/blank.html"
